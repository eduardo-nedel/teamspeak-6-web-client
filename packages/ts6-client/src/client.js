...
import { eaxEncrypt, eaxDecrypt, sha1, xorInto, deriveKeyNonce, MAC_LEN, INIT_MAC, DUMMY_KEY, DUMMY_NONCE, INIT_VERSION, hashPassword, } from "./crypto.js";
import { getSharedSecret, } from "./identity.js";
import { parseLicense, deriveLicenseKey, getSharedSecret2, generateTemporaryKey, } from "./license.js";
import { buildCommand, parseCommand } from "./commands.js";
import { qlzDecompress } from "./quicklz.js";
// Packet types
export var PacketType;
(function (PacketType) {
    PacketType[PacketType["Voice"] = 0] = "Voice";
    PacketType[PacketType["VoiceWhisper"] = 1] = "VoiceWhisper";
    PacketType[PacketType["Command"] = 2] = "Command";
    PacketType[PacketType["CommandLow"] = 3] = "CommandLow";
    PacketType[PacketType["Ping"] = 4] = "Ping";
    PacketType[PacketType["Pong"] = 5] = "Pong";
    PacketType[PacketType["Ack"] = 6] = "Ack";
    PacketType[PacketType["AckLow"] = 7] = "AckLow";
    PacketType[PacketType["Init1"] = 8] = "Init1";
...
export class Ts3Client extends EventEmitter {
    ...
    inGenerationCounter = new Uint32Array(9);
    // Resend tracking
    resendMap = new Map(); // packetId -> ResendPacket
    initResend = null;
    resendTimer = null;
    pingTimer = null;
    ...
    ivStruct = null;
    fakeSignature = Buffer.alloc(MAC_LEN, 0);
    keyNonceCache = new Map();
    alphaTmp = null;
    ...
    channelMap = new Map(); // channel_name → cid
    constructor() {
        super();
    }
    getState() {
    ...
    sendOutgoing(data, packetType, extraFlags = 0) {
    ...
    buildRawPacket(data, packetType, packetId, _gen, flags) {
    ...
    encryptPacket(raw, packetType, packetId, gen, flags, data) {
    ...
    decryptPacket(raw, packetType, packetId, gen, flags) {
        if (packetType === PacketType.Init1) {
            // Verify init MAC
            if (!INIT_MAC.equals(raw.subarray(0, MAC_LEN)))
                return null;
        ...
        if (flags & FLAG_UNENCRYPTED) {
            ...
            if (this.cryptoInitComplete) {
                if (!this.fakeSignature.equals(raw.subarray(0, MAC_LEN)))
                    return null;
            }
            return raw.subarray(MAC_LEN + S2C_HEADER_LEN);
        }
    ...
    tryDecrypt(raw, packetType, packetId, gen, forceDummy) {
    ...
    handleInit(data) {
        ...
        switch (step) {
            ...
            case 3: {
                ...
                // Build Init4
                const init4 = Buffer.alloc(4 + 1 + 64 + 64 + 4 + 100 + 64 + textBytes.length);
    ...
    async handleInitIvExpand2(params) {
