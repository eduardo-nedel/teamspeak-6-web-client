...
const OBFUSCATION_KEY = Buffer.from("b9dfaa7bee6ac57ac7b65f1094a1c155" +
    "e747327bc2fe5d51c512023fe54a2802" +
    "01004e90ad1daaae1075d53b7d571c30" +
    "e063b5a62a4a017bb394833aa0983e6e", "hex");
...
export function fromTsIdentity(identityStr) {
    const match = identityStr.match(/^(\d+)V([\w/+=]+)$/);
    if (!match)
        throw new Error("Invalid TS3 identity format");
    const keyOffset = BigInt(match[1]);
    const ident = Buffer.from(match[2], "base64");
    if (ident.length < 20)
        throw new Error("Identity too short");
...
export function fromBase64Key(base64Key, keyOffset = 0n) {
...
function importKeyFromAsn(asnData, keyOffset) {
    const parsed = parseDerSequence(asnData);
    if (parsed.length < 3)
        throw new Error("Invalid ASN.1 key data");
    const bitInfo = parsed[0];
    let privateKeyScalar = null;
    let pubX;
    let pubY;
    if (bitInfo === 0x00 || bitInfo === 0x80) {
        ...
        if (bitInfo === 0x80 && parsed.length >= 5) {
            privateKeyScalar = parsed[4];
        }
    }
    else if (bitInfo === 0xc0) {
        privateKeyScalar = parsed[2];
        pubX = 0n;
        pubY = 0n;
    }
    else {
...
export function generateIdentity(securityLevel = 8) {
    const { privateKey: ecPrivKey, publicKey: ecPubKey } = crypto.generateKeyPairSync("ec", {
        namedCurve: "prime256v1",
    });
    const jwk = ecPrivKey.export({ format: "jwk" });
...
export function generateIdentityAsync(securityLevel = 8) {
    return new Promise((resolve, reject) => {
        const workerPath = path.join(path.dirname(fileURLToPath(import.meta.url)), "identity-worker.js");
...
function improveSecurity(identity, toLevel) {
...
function getSecurityLevel(pubKeyBytes, offset) {
...
export function exportPublicKeyString(pubKey) {
    ...
    if (pointIdx < 0)
        throw new Error("Cannot extract EC point from SPKI");
...
export function getSharedSecret(privateKey, serverPublicKeyDer) {
...
function parseDerSequence(data) {
    const results = [];
    let pos = 0;
    if (data[pos] !== 0x30)
...
function buildLtcPublicKeyDer(x, y) {
...
function buildDerBigInteger(value) {
