...
export function parseLicense(data) {
    if (data.length < 1)
        throw new Error("License too short");
    if (data[0] !== 1)
        throw new Error("Unsupported license version");
    const blocks = [];
    let pos = 1;
    while (pos < data.length) {
        if (data.length - pos < 42)
            throw new Error("License block too short");
        if (data[pos] !== 0)
            throw new Error(`Wrong key kind ${data[pos]}`);
...
function findNullTerminator(data, start) {
...
export async function deriveLicenseKey(blocks) {
    await getSodium();
    let parentKey = Buffer.from(LICENSE_ROOT_KEY);
    for (const block of blocks) {
        parentKey = deriveBlockKey(block.key, block.hash, parentKey);
    }
    return parentKey;
}
function deriveBlockKey(blockKey, hash, parentKey) {
    const scalar = Buffer.from(hash);
    scalar[0] &= 0xf8;
    scalar[31] &= 0x3f;
    scalar[31] |= 0x40;
    const mulResult = Buffer.from(sodium.crypto_scalarmult_ed25519_noclamp(new Uint8Array(scalar), new Uint8Array(blockKey)));
    const result = Buffer.from(sodium.crypto_core_ed25519_add(new Uint8Array(mulResult), new Uint8Array(parentKey)));
    return result;
}
// Compute shared secret for new protocol (initivexpand2)
export async function getSharedSecret2(serverDerivedKey, tempPrivateKey) {
    await getSodium();
    const privKeyCopy = Buffer.from(tempPrivateKey);
    privKeyCopy[31] &= 0x7f;
    const mulResult = Buffer.from(sodium.crypto_scalarmult_ed25519_noclamp(new Uint8Array(privKeyCopy), new Uint8Array(serverDerivedKey)));
...
export async function generateTemporaryKey() {
    ...
    privateKey[31] &= 127;
    privateKey[31] |= 64;
    const publicKey = Buffer.from(sodium.crypto_scalarmult_ed25519_base_noclamp(new Uint8Array(privateKey)));
