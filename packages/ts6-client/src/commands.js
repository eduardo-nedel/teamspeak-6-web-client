...
export function tsUnescape(str) {
    let result = "";
...
export function buildCommand(name, params) {
    ...
    if (parts.length > 1) {
        ...
        for (let g = 1; g < parts.length; g++) {
            ...
            for (const token of gTokens) {
                ...
                if (eqIdx >= 0) {
                    gParams[token.substring(0, eqIdx)] = tsUnescape(token.substring(eqIdx + 1));
                }
                else {
                    gParams[token] = "";
                }
