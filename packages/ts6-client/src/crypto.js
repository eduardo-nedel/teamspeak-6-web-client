...
export function eaxEncrypt(key, nonce, header, plaintext, macLen = 8) {
...
export function eaxDecrypt(key, nonce, header, ciphertext, mac, macLen = 8) {
    const N = eaxOmac(key, 0, nonce);
    const H = eaxOmac(key, 1, header);
    const C = eaxOmac(key, 2, ciphertext);
    const tag = xorBuffers(xorBuffers(N, H), C);
...
export function xorInto(a, b, len) {
    for (let i = 0; i < len; i++)
        a[i] ^= b[i];
}
...
export function deriveKeyNonce(fromServer, packetId, generationId, packetType, ivStruct) {
...
export function ecdsaSign(privateKeyDer, data) {
...
export function ecdsaVerify(publicKeyDer, data, signature) {
